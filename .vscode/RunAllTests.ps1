...
{
    & git @('clone', 'https://github.com/PowerShell/DscResource.Tests.git', (Join-Path -Path $repoRoot -ChildPath '\Modules\xCertificate\DSCResource.Tests\'))
}

Import-Module (Join-Path $repoRoot "\Tests\TestHarness.psm1" -Resolve)
$dscTestsPath = Join-Path -Path $repoRoot `
    -ChildPath "\Modules\xCertificate\DscResource.Tests\Meta.Tests.ps1"
